...
if [ ! -f "$CONFIG_FILE" ]; then
    ...
    echo -e "${YELLOW}请确认您是否已经安装了 Sing-box。${PLAIN}"
    sleep 3
    exit 1
fi
...
enable_takeover() {
    ...
    if sing-box check -c "$CONFIG_FILE"; then
        ...
        if systemctl is-active --quiet sing-box; then
         ...
         LATEST_BAK=$(ls -t "${CONFIG_FILE}.bak_"* | head -1)
...
if ! command -v jq &> /dev/null; then
    echo -e "${YELLOW}正在安装 jq 依赖...${PLAIN}"
    apt-get update && apt-get install -y jq || { echo -e "${RED}jq 安装失败，请手动安装！${PLAIN}"; exit 1; }
fi
...
enable_takeover() {
    ...
    echo -e "\n${CYAN}>>> 第三步：请选择流量接管模式 <<<${PLAIN}"
    echo "1. 按【入站端口】接管 (例如让 10000 端口的流量全部走家宽)"
    echo "2. 按【入站 Tag】接管 (例如让 tag 为 'in-shadowsocks' 的流量走家宽)"
    read -p "请选择 [1-2]: " mode_choice
    ...
    if sing-box check -c "$CONFIG_FILE"; then
        ...
        if systemctl is-active --quiet sing-box; then
         ...
         echo -e "${RED}❌ 新生成的配置存在语法错误，Sing-box 拒绝加载。已自动回滚配置！${PLAIN}"
         mv "${CONFIG_FILE}.bak_$(date +%s)" "$CONFIG_FILE" # 回滚逻辑可以更精细，这里简单处理
    fi
}
...
show_menu() {
    clear
    echo -e "${GREEN}===========================================${PLAIN}"
    echo -e "${GREEN}    Sing-box 家宽流量接管管理模块 (多协议增强版)    ${PLAIN}"
    echo -e "${GREEN}===========================================${PLAIN}"
    echo -e " 1. ${GREEN}开启/修改${PLAIN} 家宽流量接管 (支持 SS/Trojan/Socks/HTTP/VMess)"
    echo -e " 2. ${RED}关闭/移除${PLAIN} 家宽流量接管"
    echo -e " 0. 退出脚本"
